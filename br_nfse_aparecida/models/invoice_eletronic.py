...
try:
    from pytrustnfe.nfse.aparecida import xml_recepcionar_lote_rps
    from pytrustnfe.nfse.aparecida import recepcionar_lote_rps
    from pytrustnfe.nfse.aparecida import consultar_lote_rps
    from pytrustnfe.nfse.aparecida import cancelar_nfse
...
class InvoiceEletronic(models.Model):
    ...
    model = fields.Selection(
        selection_add=[('016', 'NFS-e - Aparecida, SP')])
    ...
    def _find_attachment_ids_email(self):
        ...
        danfe_report = self.env['ir.actions.report'].search(
            [('report_name', '=',
              'br_nfse_aparecida.main_template_br_nfse_danfe_aparecida')])
