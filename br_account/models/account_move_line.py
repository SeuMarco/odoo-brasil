...
=======
>>>>>>> d853bc40... 2to3 + removes de.get_precision functions and theis imports
...
>>>>>>> d853bc40... 2to3 + removes de.get_precision functions and theis imports
    ...
    def _compute_price(self):
        ...
        if self.invoice_line_tax_ids:
            ...
            taxes = tax_ids.compute_all(price,
                                        currency,
                                        self.quantity,
        ...
        icms_inter = ([
            x for x in taxes['taxes'] if x['id'] == self.tax_icms_inter_id.id
        ]) if taxes else []
        icms_intra = ([
            x for x in taxes['taxes'] if x['id'] == self.tax_icms_intra_id.id
        ]) if taxes else []
        icms_fcp = (
            [x for x in taxes['taxes']
        ...
        self.update({
            'valor_liquido':
            taxes['total_included'] if taxes else subtotal,
            'price_tax':
            taxes['total_included'] - taxes['total_excluded'] if taxes else 0,
            'price_subtotal':
            taxes['total_excluded'] if taxes else subtotal,
            ...
            'icms_valor':
            sum([x['amount'] for x in icms]),
            'icms_st_base_calculo':
            sum([x['base'] for x in icmsst]),
            'icms_st_valor':
            sum([x['amount'] for x in icmsst]),
            'icms_bc_uf_dest':
            sum([x['base'] for x in icms_inter]),
            'icms_uf_remet':
            sum([x['amount'] for x in icms_inter]),
            'icms_uf_dest':
            sum([x['amount'] for x in icms_intra]),
            'icms_fcp_uf_dest':
            sum([x['amount'] for x in icms_fcp]),
            'icms_valor_credito':
            base_icms_credito * (self.icms_aliquota_credito / 100),
            'ipi_base_calculo':
            sum([x['base'] for x in ipi]),
            'ipi_valor':
            sum([x['amount'] for x in ipi]),
            'pis_base_calculo':
            sum([x['base'] for x in pis]),
            'pis_valor':
            sum([x['amount'] for x in pis]),
            'cofins_base_calculo':
            sum([x['base'] for x in cofins]),
            'cofins_valor':
            sum([x['amount'] for x in cofins]),
            'issqn_base_calculo':
            sum([x['base'] for x in issqn]),
            'issqn_valor':
            sum([x['amount'] for x in issqn]),
            'ii_base_calculo':
            sum([x['base'] for x in ii]),
            'ii_valor':
            sum([x['amount'] for x in ii]),
            'csll_base_calculo':
            sum([x['base'] for x in csll]),
            'csll_valor':
            sum([x['amount'] for x in csll]),
            'inss_base_calculo':
            sum([x['base'] for x in inss]),
            'inss_valor':
            sum([x['amount'] for x in inss]),
            'irrf_base_calculo':
            sum([x['base'] for x in irrf]),
            'irrf_valor':
            sum([x['amount'] for x in irrf]),
        })
...
>>>>>>> d853bc40... 2to3 + removes de.get_precision functions and theis imports
        ...
        required=True,
        default='4')
    icms_st_valor = fields.Float('Valor ICMS ST',
                                 required=True,
                                 compute='_compute_price',
                                 store=True,
<<<<<<< HEAD
...
>>>>>>> d853bc40... 2to3 + removes de.get_precision functions and theis imports
    ...
    pis_tipo = fields.Selection([('percent', 'Percentual')],
                                ...
                                required=True,
                                default='percent')
    pis_base_calculo = fields.Float('Base PIS',
                                    required=True,
                                    compute='_compute_price',
                                    store=True,
<<<<<<< HEAD
