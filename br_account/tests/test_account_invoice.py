...
class TestAccountInvoice(TestBaseBr):
    ...
    def test_invoice_pis_cofins_taxes(self):
        for invoice in self.invoices:
            ...
            self.assertEqual(first_item.valor_liquido, 150.0)
            ...
            for item in invoice.invoice_line_ids:
                ...
                self.assertEqual(item.pis_base_calculo, item.valor_liquido)
                self.assertEqual(item.pis_aliquota, 5.0)
                self.assertEqual(item.pis_valor, item.valor_liquido * 0.05)

                item.tax_cofins_id = self.cofins_1500
                item._onchange_tax_cofins_id()
                item._onchange_product_id()
                self.assertEqual(item.cofins_base_calculo, item.valor_liquido)
                self.assertEqual(item.cofins_aliquota, 15.0)
                self.assertEqual(item.cofins_valor, item.valor_liquido * 0.15)
    ...
    def test_invoice_issqn_and_ii_taxes(self):
        for invoice in self.invoices:
            ...
            self.assertEqual(prod_item.valor_liquido, 150.0)
            ...
            self.assertEqual(serv_item.valor_liquido, 500.0)
    ...
    def test_invoice_icms_normal_tax(self):
        for invoice in self.invoices:
            ...
            for item in invoice.invoice_line_ids:
                ...
                item._onchange_product_id()
                self.assertEqual(item.icms_base_calculo, item.valor_liquido)
                self.assertEqual(
