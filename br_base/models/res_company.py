...
class ResCompany(models.Model):
    ...
    def _compute_expiry_date(self):
        for rec in self:
            try:
                ...
                _logger.warning(_(
                    'Unknown error when validating certificate. %s', exc),
                    exc_info=True)
    ...
    cert_state = fields.Selection([('not_loaded', _("Not loaded")),
                                   ('expired', _("Expired")),
                                  ...
                                  default='not_loaded', store=True)
    cert_information = fields.Text(string=_("Cert. Info"),
                                   compute=_compute_expiry_date,
                                   default=_("Not Loaded"), store=True)
    cert_expire_date = fields.Date(string=_("Cert. Expiration Date"),
